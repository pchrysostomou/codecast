...
// ── Body parser (for /api/run POST requests) ──────────────────
app.use(express.json({ limit: '256kb' }))

// ── POST /api/run  — cloud code execution via Wandbox ─────────
// Called by Vercel /api/run when language is not JS/TS.
// Railway has no serverless timeout, so Wandbox compilation always completes.
const WANDBOX: Record<string, { compiler: string; options?: string }> = {
  python: { compiler: 'cpython-head' },
  java:   { compiler: 'openjdk-head' },
  go:     { compiler: 'go-head' },
  c:      { compiler: 'gcc-head' },
  cpp:    { compiler: 'gcc-head', options: 'c++17,warning' },
...
app.post('/api/run', async (req, res) => {
  ...
  const cfg = WANDBOX[language?.toLowerCase()]
  if (!cfg) {
    res.json({
      stdout: '',
      stderr: `'${language}' is not supported. Available: python, java, go, c, cpp, rust, bash`,
      exitCode: 1, language, runtime: 0,
    })
    return
  }
  ...
  if (cfg.options) payload['options'] = cfg.options
  ...
  try {
    const wandboxRes = await fetch('https://wandbox.org/api/compile.json', {
      ...
      signal: AbortSignal.timeout(28_000),  // 28s — Railway container has no function timeout
    })

    if (!wandboxRes.ok) {
      res.json({ stdout: '', stderr: `Wandbox HTTP ${wandboxRes.status}`, exitCode: 1, language, runtime: Date.now() - t0 })
      return
    }
    ...
    console.log(`[run] ${language} via Wandbox in ${Date.now() - t0}ms exit=${exitCode}`)
    res.json({ stdout, stderr, exitCode, language, runtime: Date.now() - t0 })
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
    console.error(`[run] ${language} error:`, msg)
    res.json({ stdout: '', stderr: `Execution error: ${msg}`, exitCode: 1, language, runtime: Date.now() - t0 })
  }
})
