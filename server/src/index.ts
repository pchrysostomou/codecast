...
// ── POST /api/run — cloud code execution (no timeout limits) ──
// Tried in order: Wandbox (free, no key), then friendly error
...
app.post('/api/run', async (req, res) => {
  ...
  const cfg = WANDBOX_MAP[language]
  if (!cfg) {
    res.json({ stdout: '', stderr: `Language '${language}' is not directly supported for execution.`, exitCode: 1, language, runtime: 0 })
    return
  }
  ...
  try {
    const wandboxRes = await fetch('https://wandbox.org/api/compile.json', {
      ...
      signal: AbortSignal.timeout(28000),  // 28s — Railway has no timeout
    })

    if (!wandboxRes.ok) {
      res.json({ stdout: '', stderr: `Compiler service returned HTTP ${wandboxRes.status}`, exitCode: 1, language, runtime: Date.now() - t0 })
      return
    }
    ...
    res.json({ stdout: '', stderr: `Execution service error: ${msg}`, exitCode: 1, language, runtime: Date.now() - t0 })
  }
})
